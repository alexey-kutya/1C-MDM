Процедура ВыполнитьРегистрациюДляУПП(Ссылка) экспорт
	...
	Если ТаблицаУзловОбмена.Количество() Тогда
		...
		Если УзлыОбменаРегистрация.Количество() Тогда
			...
			Если ПолнаяСинхронизация Тогда
				...
				Сообщить("Выполнена регистрация: "+Ссылка);
			Иначе
		...
		УзлыОбменаУдаление = ПолучитьМассивУзлов(ТаблицаУзловОбмена, Ложь); //если вдруг сняли признак регистрации с узла, очистим изменения
	...
	ПланыОбмена.Обмен_MDM_AX.ВыполнитьРегистрациюДляAX(Ссылка);
...
Процедура ВыполнитьРегистрациюКонтактнойИнформации(НаборЗаписейКИ, УзлыОбмена, Регистрация) Экспорт 
	...
	Для каждого СтрокаИзмерений Из ИзмеренияТЗ Цикл
		...
		Если Регистрация Тогда
			...
			Сообщить("Выполнена регистрация контактной информации: "+СтрокаИзмерений.Объект+", "+СтрокаИзмерений.Вид);
		Иначе
...
Процедура ВыполнитьРегистрациюПодчиненногоСправочника(Владелец, УзлыОбмена, Регистрация, ИмяСправочника, УдалениеИзУзловОбмена = Ложь)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ПолнаяСинхронизация 
			...
			Сообщить("Выполнена регистрация подчиненного справочника: "+Выборка.Ссылка);
		Иначе
...
Функция ПолучитьУзелПоОрганизации(Организация) Экспорт 

	Возврат ПланыОбмена.Обмен_МДМ_УПП.НайтиПоРеквизиту("Организация", Организация);

КонецФункции // ПолучитьУзелПоОрганизации()
