...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидСправочника.ИспользоватьЗаявки ИЛИ ВидСправочника.ИспользоватьНормализацию Тогда 
		...
		ДопСвязи = "
			...
			|	И ОсновнаяТаблица.ТипПозиции = ЗНАЧЕНИЕ(Перечисление.нсиТипыПозицийСправочников.ЭталоннаяПозиция)";
	КонецЕсли;
	...
	Если ВидСправочника.ИспользоватьЗаявки Тогда 
...
&НаСервере
Процедура НастроитьСписокДублей()
	СписокДублей.ТекстЗапроса = 
		СтрЗаменить(Список.ТекстЗапроса,
			"нсиТипыПозицийСправочников.ЭталоннаяПозиция",
			"нсиТипыПозицийСправочников.ДублирующаяПозиция")
		+"
		|	И ОсновнаяТаблица.ЭталоннаяПозиция = &ЭталоннаяПозиция";
	
	Элементы.СписокДублей.КартинкаСтрок = БиблиотекаКартинок.нсиПиктограммыЭлементов;
	Элементы.СписокДублей.ПутьКДаннымКартинкиСтроки = "СписокДублей.КартинкаСтроки";
	СписокДублей.Параметры.УстановитьЗначениеПараметра("ВидСправочника", пМетаданные.ВидСправочника);
	Для каждого КлючИЗначение Из пМетаданные.Реквизиты Цикл
		Имя = КлючИЗначение.Ключ;
		пРеквизит = КлючИЗначение.Значение;
		
		пРеквизит = КлючИЗначение.Значение;
		СписокДублей.Параметры.УстановитьЗначениеПараметра("Р"+Имя, пРеквизит.Идентификатор);
	КонецЦикла;
	
	нсиУниверсальноеХранилищеФормыСервер.УстановитьОформлениеСписка(СписокДублей.КомпоновщикНастроек.Настройки.УсловноеОформление,пМетаданные);
	
	Поле = Элементы.Добавить("СписокДублейКод", Тип("ПолеФормы"), Элементы.СписокДублей);
	Поле.ПутьКДанным = "СписокДублей.Код";
	Поле = Элементы.Добавить("СписокДублейНаименование", Тип("ПолеФормы"), Элементы.СписокДублей);
	Поле.ПутьКДанным = "СписокДублей.Наименование";
	
	Для каждого КлючИЗначение Из пМетаданные.Реквизиты Цикл
		Имя = КлючИЗначение.Ключ;
		пРеквизит = КлючИЗначение.Значение;
		
		Поле = Элементы.Добавить("СписокДублей"+Имя, Тип("ПолеФормы"), Элементы.СписокДублей);
		Если пРеквизит.Тип.Тип1="Булево" Тогда
			Поле.Вид = ВидПоляФормы.ПолеФлажка;
		КонецЕсли;
		Поле.ПутьКДанным = "СписокДублей."+Имя;
		Поле.Заголовок = пРеквизит.Наименование;
	КонецЦикла;
	
	Элементы.СписокДублей.Отображение = ОтображениеТаблицы.Список;
	СписокДублей.Параметры.УстановитьЗначениеПараметра("ЭталоннаяПозиция", Справочники.нсиМТР.ПустаяСсылка());
	СписокДублей.Параметры.УстановитьЗначениеПараметра("СсылкаНеОпределена", 	Истина);
	СписокДублей.Параметры.УстановитьЗначениеПараметра("Ссылка", Null);
