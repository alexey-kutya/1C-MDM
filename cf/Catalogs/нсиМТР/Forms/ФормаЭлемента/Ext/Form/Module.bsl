...
Процедура ЗапросGlobalID(Команда)
	...
	РучнойРежим = Команда.Имя = "СоздатьИзменитьGlobalБезЗапроса";
	...
	Если ЗначениеЗаполнено(GlobalSKU) Тогда
		State = ЗначениеРеквизитаОбъекта(GlobalSKU, "State");
		...
		Если State = ПредопределенноеЗначение("Перечисление.States.InProgress") Тогда
			...
			ОткрытьФорму("Справочник.GlobalSKU.ФормаОбъекта", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
//			ОткрытьФорму("ВнешняяОбработка.ЗапросGlobal.Форма.ФормаЭлементаGlobal", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
		ИначеЕсли State = ПредопределенноеЗначение("Перечисление.States.Rejected") Тогда
			...
			ОткрытьФорму("Справочник.GlobalSKU.ФормаОбъекта", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
//			ОткрытьФорму("ВнешняяОбработка.ЗапросGlobal.Форма.ФормаЭлементаGlobal", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
		ИначеЕсли State = ПредопределенноеЗначение("Перечисление.States.Approved")
					И НЕ РучнойРежим Тогда
//080819			
			ОткрытьФорму("Справочник.GlobalSKU.ФормаОбъекта", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
//			ОткрытьФорму("ВнешняяОбработка.ЗапросGlobal.Форма.ФормаЭлементаGlobal", Новый Структура("Ключ", GlobalSKU),ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма));
		Иначе
			ПараметрыДляФормы = ЗаполнитьАтрибутыGlobal(Объект.Ссылка, LocalFactory, GlobalSKU,РучнойРежим);
//080819	
			ОткрытьФорму("Справочник.GlobalSKU.ФормаОбъекта",ПараметрыДляФормы,ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма,ТекущиеДанныеGlobalID));
//			ОткрытьФорму("ВнешняяОбработка.ЗапросGlobal.Форма.ФормаЭлементаGlobal",ПараметрыДляФормы,ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма,ТекущиеДанныеGlobalID));
		КонецЕсли; 
	Иначе
		ПараметрыДляФормы = ЗаполнитьАтрибутыGlobal(Объект.Ссылка, LocalFactory,,РучнойРежим);
//080819	
		ОткрытьФорму("Справочник.GlobalSKU.ФормаОбъекта",ПараметрыДляФормы,ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма,ТекущиеДанныеGlobalID));
//		ОткрытьФорму("ВнешняяОбработка.ЗапросGlobal.Форма.ФормаЭлементаGlobal",ПараметрыДляФормы,ЭтаФорма,,,,Новый ОписаниеОповещения("ПриЗакрытииФормыGlobalSKU",ЭтаФорма,ТекущиеДанныеGlobalID));
	КонецЕсли;
...
//
&НаКлиенте
Функция ЗаполнитьАтрибутыGlobal(LocalSKU, LocalFactory, Ключ = Неопределено, РучнойРежим = Ложь)
	
	СтруктураGlobalSKU = ПолучитьСтруктуруGlobalSKU(LocalSKU, LocalFactory);
	
	ПараметрыДляФормы = Новый Структура;
	ПараметрыДляФормы.Вставить("РучнойРежим", РучнойРежим);
...
&НаСервереБезКонтекста
Функция ПолучитьСтруктуруGlobalSKU(LocalSKU, LocalFactory)

	Возврат MDMСервер.interface(LocalSKU).СтруктураСоответствияАтрибутовGlobal(LocalSKU, LocalFactory);

КонецФункции // ПолучитьСтруктуруGlobalSKU()
  
&НаКлиенте
Процедура ПриЗакрытииФормыGlobalSKU(Ответ, ДополнительныеПараметры) Экспорт 

	Если ЗначениеЗаполнено(Ответ)
		И НЕ ЗначениеЗаполнено(ДополнительныеПараметры.GlobalSKU) Тогда
		Объект.GlobalID[ДополнительныеПараметры.НомерСтроки-1].GlobalSKU = Ответ;
...
&НаСервереБезКонтекста
Функция ProtectedStates(State)
	Возврат НЕ GlobEx.ProtectedStates().Найти(State) = Неопределено;
КонецФункции // ProtectedStates()
	
&НаКлиенте
Процедура GlobalIDПередУдалением(Элемент, Отказ)
	Отказ = ProtectedStates(ЗначениеРеквизитаОбъекта(Элемент.ТекущиеДанные.GlobalSKU, "State"));
КонецПроцедуры

&НаКлиенте
Процедура GlobalIDПередНачаломИзменения(Элемент, Отказ)
	Отказ = ProtectedStates(ЗначениеРеквизитаОбъекта(Элемент.ТекущиеДанные.GlobalSKU, "State"));
КонецПроцедуры
...
&НаКлиенте
Процедура ЭкспортДоступность()

	ЭкспортДоступность = ЭкспортДоступностьНаСервере();
	Элементы.алкЭкспорт.Доступность = ЭкспортДоступность;
	Если НЕ ЭкспортДоступность Тогда
		Объект.алкЭкспорт = Ложь;
		Объект.Дистрибьютор = ПредопределенноеЗначение("Справочник.нсиКонтрагенты.ПустаяСсылка");
	КонецЕсли; 
	ДистрибьюторВидимость();

КонецПроцедуры // ЭкспортДоступность()
 
&НаСервере
Функция ЭкспортДоступностьНаСервере()

	Возврат НЕ (Объект.LocalMaterialType.ВидМатериалаSAP.Код = "ZHAW" И НЕ Объект.СтранаПроисхождения.КодАльфа2 = "RU");
...
Процедура ИзменитьСтатусGlobal(Команда)
	...
	Если Параметры.Ключ.Пустая()
		ИЛИ Модифицированность Тогда
		Сообщить("Перед изменением статуса Global SKU необходимо записать объект.", СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(GlobalSKU) Тогда
		State = ЗначениеРеквизитаОбъекта(GlobalSKU, "State");
		Status = ЗначениеРеквизитаОбъекта(GlobalSKU, "Status");
		Если State = ПредопределенноеЗначение("Перечисление.States.Approved")
			И Status = ПредопределенноеЗначение("Перечисление.SKUStatuses.ACTIVE") Тогда
			ПоказатьВводЗначения(Новый ОписаниеОповещения("ОбработатьВыборСтатуса",ЭтотОбъект,GlobalSKU), ПредопределенноеЗначение("Перечисление.SKUStatuses.INACTIVE"),"Выберите новый статус",Тип("ПеречислениеСсылка.SKUStatuses"));
		Иначе
			Сообщить("Невозможно выполнить действие. Условие для выполнения State = Approved, Global status = ACTIVE.");
		КонецЕсли; 
	Иначе
		Сообщить("Невозможно выполнить действие. Не заполнен Global SKU.");
	КонецЕсли; 
		
КонецПроцедуры

&НаКлиенте
Процедура ОбработатьВыборСтатуса(ВыбранныйСтатус, GlobalSKU) Экспорт 
	
	Если ВыбранныйСтатус = ПредопределенноеЗначение("Перечисление.SKUStatuses.INACTIVE")
		ИЛИ ВыбранныйСтатус = ПредопределенноеЗначение("Перечисление.SKUStatuses.DELISTED") Тогда
		
		Если ИзменитьСтатусGlobalНаСервере(GlobalSKU, ВыбранныйСтатус) Тогда
			ЭтаФорма.Прочитать();
			ОповеститьОбИзменении(GlobalSKU); 
		КонецЕсли; 
	Иначе
		Сообщить("Невозможно выполнить действие. Новый статус должен быть INACTIVE или DELISTED.");
	КонецЕсли; 
	
КонецПроцедуры // ()

&НаСервереБезКонтекста
Функция ИзменитьСтатусGlobalНаСервере(GlobalSKU, ВыбранныйСтатус)

	Возврат GlobEx.ИзменитьСтатусGlobal(GlobalSKU, ВыбранныйСтатус);

КонецФункции // ИзменитьСтатусGlobalНаСервере()

//ОТЗЫВ ЗАПРОСА GLOBAL
&НаСервереБезКонтекста
Функция ОтозватьЗапросGlobalIDНаСервере(GlobalSKU)
	Возврат GlobEx.ОтозватьЗапросGlobalID_SQL(GlobalSKU);
КонецФункции
...
Процедура ОтозватьЗапросGlobalID(Команда)
	...
	Если ЗначениеЗаполнено(GlobalSKU) Тогда
		...
		Status = ЗначениеРеквизитаОбъекта(GlobalSKU, "Status");
		Если State = ПредопределенноеЗначение("Перечисление.States.InProgress") Тогда
			Если ОтозватьЗапросGlobalIDНаСервере(GlobalSKU) Тогда
				ЭтаФорма.Прочитать();
				ОповеститьОбИзменении(GlobalSKU); 
...
Функция ДеактивацияGlobalНаСервере(СсылкаGlobal)
	...
	Возврат GlobEx.ДеактивацияGlobal(СсылкаGlobal, Перечисления.SKUStatuses.INACTIVE);
...
Процедура ДеактивацияGlobal(Команда)
	...
	Если ЗначениеЗаполнено(GlobalSKU) Тогда
		...
		Если State = ПредопределенноеЗначение("Перечисление.States.ПустаяСсылка")
			...
			ПоказатьВопрос(Новый ОписаниеОповещения("ПослеЗакрытияВопросаДеактивации", ЭтотОбъект, GlobalSKU),ТекстВопроса,РежимДиалогаВопрос.ДаНет,,КодВозвратаДиалога.Нет,"Деактивация записи Global");
			Возврат;
		Иначе
