...
//КутьяАА ITRR
&НаСервереБезКонтекста
Процедура СоздатьЕдиницуИзмерения(Владелец, ЕдиницаИзмерения)

	НайденнаяЕдиница = Справочники.ЕдиницыИзмерения.НайтиПоРеквизиту("ЕдиницаПоКлассификатору", ЕдиницаИзмерения, ,Владелец);
	Если НайденнаяЕдиница.Пустая() Тогда
		...
		НайденнаяЕдиницаОбъект.Владелец                = Владелец;
...
//КутьяАА ITRR отключено по просьбе сотрудника отдела мастер даных
	Если ЗначениеЗаполнено(ТекущийОбъект.БазоваяЕдиницаИзмерения) Тогда
		
		//отключено по просьбе сотрудника отдела мастер даных
		//ПроверитьЕдиницуХраненияОстатков(ТекущийОбъект);
		СоздатьЕдиницуИзмерения(ТекущийОбъект.Ссылка, ТекущийОбъект.БазоваяЕдиницаИзмерения);
				
		ГруппаМатериаловХранилище = ПолучитьГруппуМатериаловАвтоматическиСоздаваемыхЕдиниц();
		
		Если НЕ ГруппаМатериаловХранилище = Неопределено Тогда
			
			ГруппаМатериалов = ГруппаМатериаловХранилище.Получить();
			
			Если ЗначениеЗаполнено(ТекущийОбъект.LocalMaterialType)
				И ТекущийОбъект.LocalMaterialType.ПринадлежитЭлементу(ГруппаМатериалов) Тогда
				
				СписокХранилище = ПолучитьСписокАвтоматическиСоздаваемыхЕдиниц();
				Если НЕ СписокХранилище = Неопределено Тогда
					СписокЕдиниц = СписокХранилище.Получить();
					Для каждого СтрокаТаблицы Из СписокЕдиниц Цикл
						
						СоздатьЕдиницуИзмерения(ТекущийОбъект.Ссылка, СтрокаТаблицы.Единица);
...
Процедура ЗапросGlobalID(Команда)
	...
	Если ЗначениеЗаполнено(GlobalSKU) Тогда
		Сообщить("Запрос не может быть выполнен. Global SKU уже задан.", СтатусСообщения.Важное);
		Возврат;
	КонецЕсли;
		
	СтруктураGlobalSKU = СтруктураСоответствияАтрибутовGlobal(Объект.Ссылка, LocalFactory);
...
Функция СтруктураСоответствияАтрибутовGlobal(НоменклатураСсылка, LocalFactory)
	...
	СтруктураGlobalSKU.Вставить("LocalName", НоменклатураСсылка.Наименование);
