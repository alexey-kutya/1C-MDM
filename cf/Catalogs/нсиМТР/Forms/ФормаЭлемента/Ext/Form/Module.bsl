...
&НаСервереБезКонтекста
Функция ОбъемДКЛ(ОбъемЛ, Коэффициент)
	...
	Если Единица.Пустая() Тогда
		...
		Если Владелец.LocalMaterialType.ГотоваяПродукция Тогда
			...
			Для каждого СтрокаGlobalID Из Объект.GlobalID Цикл
				...
				СтрокаЕдПроизв.Объем = ОбъемДКЛ(Объект.LocalVolume, 1);
			КонецЦикла;
		...
		Для каждого СтрокаЕдИзм Из НастройкиЕдиницИзмерения.Настройка.СписокЕдиниц Цикл
			...
			Если ЗначениеЗаполнено(СтрокаЕдИзм.Единица) Тогда
				...
				Если Единица.Пустая() Тогда
					...
					ЕдиницаОбъект = СоздатьЕдиницуИзмерения(Объект.Ссылка, СтрокаЕдИзм.Единица, ТаблицаПроизводителей);
					
					Единица = ЕдиницаОбъект.Ссылка;
				...
				Если ЗначениеЗаполнено(Единица) Тогда
					...
					Отбор.Вставить("ЕдиницаПоКлассификатору", СтрокаЕдИзм.Единица);
					
					СтрокиЕдиницНазначения = НастройкиЕдиницИзмерения.Настройка.НазначениеЕдиниц.НайтиСтроки(Отбор);
					Для каждого СтрокаЕдиницыНазначения Из СтрокиЕдиницНазначения Цикл
						Если НЕ ЗначениеЗаполнено(Объект[СтрокаЕдиницыНазначения.Назначение]) Тогда
