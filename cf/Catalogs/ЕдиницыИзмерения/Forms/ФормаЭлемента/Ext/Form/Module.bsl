...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокКоэффициент = "Содержит ("+Объект.Владелец.БазоваяЕдиницаИзмерения.Наименование+")";
	Элементы.Коэффициент.Заголовок = ЗаголовокКоэффициент;
	Элементы.ЕдиницыПроизводителейКоэффициент.Заголовок = ЗаголовокКоэффициент;
...
&НаКлиенте
Процедура ЕдиницыПроизводителейКоэффициентПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.ЕдиницыПроизводителей.ТекущиеДанные;
	Объект.ЕдиницыПроизводителей[ТекущиеДанные.НомерСтроки-1].Объем = ПолучитьОбъемДклНаСервере(Объект.Владелец, ТекущиеДанные.Коэффициент);
	
КонецПроцедуры

&НаСервере
Функция ПолучитьОбъемДклНаСервере(НоменклатураСсылка, Коэффициент)

	Возврат Справочники.нсиМТР.ОбъемДКЛ(НоменклатураСсылка.LocalVolume, Коэффициент);

КонецФункции // ЗаполнитьОбъемДклНаСервере()
