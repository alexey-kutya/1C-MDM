...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если не Отказ Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		               |	нсиКлассификаторПродукции.Ссылка
		               |ИЗ
		               |	Справочник.нсиКлассификаторПродукции КАК нсиКлассификаторПродукции
		               |ГДЕ
		               |	нсиКлассификаторПродукции.Ссылка В ИЕРАРХИИ(&Ссылка)
		               |	И НЕ нсиКлассификаторПродукции.Ссылка = &Ссылка
		               |	И (НЕ нсиКлассификаторПродукции.Методолог = &Методолог
		               |			ИЛИ НЕ нсиКлассификаторПродукции.Эксперт = &Эксперт)";
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если Объект.ЭтоМакет Тогда 
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("Запись макета элемента справочника ""Классификатор продукции""");
	Иначе
		ОценкаПроизводительностиКлиентСервер.НачатьЗамерВремени("Запись элемента справочника ""Классификатор продукции""");
	КонецЕсли;
КонецПроцедуры
...
Процедура ОпределитьТипЭлементаНаименования(ДанныеСтроки)
	...
	Если ДанныеСтроки.ТипЭлемента = ПредопределенноеЗначение("Перечисление.нсиТипыЭлементовНаименования.Реквизит") тогда
		...
		Если ЗначениеЗаполнено(ДанныеСтроки.ЭлементНаименования) 
			И Элементы.НастройкаШаблоновНаименованийЭлементНаименования.СписокВыбора.НайтиПоЗначению(ДанныеСтроки.ЭлементНаименования) = Неопределено тогда
	...
	ИначеЕсли ДанныеСтроки.ТипЭлемента = ПредопределенноеЗначение("Перечисление.нсиТипыЭлементовНаименования.Характеристика") тогда
		...
		ОписаниеТипа = Новый ОписаниеТипов("ПланВидовХарактеристикСсылка.нсиХарактеристикиМТР");
		ДанныеСтроки.ЭлементНаименования = ОписаниеТипа.ПривестиЗначение(ДанныеСтроки.ЭлементНаименования);
...
Функция СписокРеквизитов()
	...
	Для Каждого Реквизит из Метаданные.Справочники.нсиМТР.Реквизиты цикл
...
Функция СписокДополнительныхРеквизитов()
	...
	Запрос.УстановитьПараметр("Набор", Справочники.НаборыДополнительныхРеквизитовИСведений.Справочник_нсиМТР_Общие);
