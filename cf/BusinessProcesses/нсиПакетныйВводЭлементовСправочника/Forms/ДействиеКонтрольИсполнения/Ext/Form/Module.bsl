...
Процедура Выполнено(Команда)
	...
	Для Каждого СтрокаТЧ Из Задание.Предметы Цикл 
		Если СтрокаТЧ.Статус = СтатусОбработано Тогда 
	...
	Отклонено = (Задание.Предметы.НайтиСтроки(новый Структура("Статус",ПредопределенноеЗначение("Перечисление.нсиСтатусыОбработкиЗаявок.Отклонено"))).Количество() 
				+Задание.Предметы.НайтиСтроки(новый Структура("Статус",ПредопределенноеЗначение("Перечисление.нсиСтатусыОбработкиЗаявок.Отозвано"))).Количество())
				= Задание.Предметы.Количество();
				
	НадоУточнить = Задание.Предметы.НайтиСтроки(новый Структура("Статус",ПредопределенноеЗначение("Перечисление.нсиСтатусыОбработкиЗаявок.ЕстьОшибки"))).Количество() <> 0;
...
Функция НадоОчиститьКомментарий(Задание,Предмет,Статус)
	...
	Если Выборка.Следующий() Тогда 
		Возврат 
			Выборка.Статус <> Перечисления.нсиСтатусыОбработкиЗаявок.Отклонено
			И Выборка.Статус <> Перечисления.нсиСтатусыОбработкиЗаявок.ЕстьОшибки;
	Иначе
		Возврат Ложь;
	КонецЕсли;
КонецФункции
