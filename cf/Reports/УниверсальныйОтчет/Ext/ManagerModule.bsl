...
Функция ИнициализироватьОтчет(КомпоновщикНастроек,
	...
	Иначе
		...
		// КутьяАА <<
		Если ПараметрыОтчета.ТипОбъектаМетаданных = "Справочники" Тогда
			ВычисляемоеПоле = СКД.ВычисляемыеПоля.Добавить();
			ВычисляемоеПоле.Выражение = "XMLСтрока(Ссылка)";
			ВычисляемоеПоле.ПутьКДанным = "GUID";
			ВычисляемоеПоле.Заголовок = "GUID";
		КонецЕсли;
		// >> КутьяАА 
...
Процедура ДобавитьПоказатели(ПараметрыОтчета, КомпоновщикНастроек)
	...
	ИначеЕсли ПараметрыОтчета.ТипОбъектаМетаданных = "Документы" 
		...
		// КутьяАА <<
		Если ПараметрыОтчета.ТипОбъектаМетаданных = "Справочники" Тогда
			ОтчетыКлиентСервер.ДобавитьВыбранноеПоле(ВыбранныеПоля, "GUID");
		КонецЕсли; 
		// >> КутьяАА 
		Для каждого Реквизит Из ОбъектМетаданных.Реквизиты Цикл
