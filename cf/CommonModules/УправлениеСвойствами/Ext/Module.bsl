...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	//нси. Работа с универсальным хранилищем
	УправлениеСвойствамиПереопределяемый.нсиДополнитьОтборамиУниверсальногоХранилища(Форма);	
...
Процедура ПередЗаписьюВидаОбъекта(ВидОбъекта,
	...
	Если ТипЗнч(ВидОбъекта)=Тип("СправочникОбъект.нсиМТР") И ВидОбъекта.ЭтоГруппа Тогда
		Возврат;
	КонецЕсли;
	//--
