...
//	ITRR Кутья АА добавлена локализация для английского интерфейса
	...
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("КодВозвратаДиалога.Да",  НСтр("ru = 'Завершить'; en = 'Exit'"));
	Кнопки.Добавить("КодВозвратаДиалога.Нет", НСтр("ru = 'Отмена'; en = 'Cancel'"));
	
	ПараметрыВопроса = ПараметрыВопросаПользователю();
	ПараметрыВопроса.БлокироватьВесьИнтерфейс = Истина;
	ПараметрыВопроса.КнопкаПоУмолчанию = "КодВозвратаДиалога.Да";
	ПараметрыВопроса.Заголовок = НСтр("ru = 'Завершение работы'; en = 'Shutdown'");
	ПараметрыВопроса.БольшеНеЗадаватьЭтотВопрос = Ложь;
	
	ПоказатьВопросПользователю(ОбработкаОтвета, НСтр("ru = 'Завершить работу с программой?'; en = 'Exit the program?'"), Кнопки, ПараметрыВопроса);
