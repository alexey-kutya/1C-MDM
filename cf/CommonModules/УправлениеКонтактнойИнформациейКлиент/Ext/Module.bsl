...
Процедура ПриИзменении(Форма, Элемент, ЭтоТабличнаяЧасть = Ложь) Экспорт
	...
	//КутьяАА ITRR <<
	Если ДанныеСтроки.Тип = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты") Тогда
		Отбор = Новый Структура("Вид", ДанныеСтроки.Вид);
		Строки = Форма.КонтактнаяИнформацияУПП.НайтиСтроки(Отбор);
	
		Если Строки.Количество() = 0 Тогда
			ДанныеСтрокиУПП = Форма.КонтактнаяИнформацияУПП.Добавить();
			ДанныеСтрокиУПП.Объект = Форма.Параметры.Ключ;
		Иначе	
			ДанныеСтрокиУПП = Строки[0];
		КонецЕсли; 
		ЗаполнитьЗначенияСвойств(ДанныеСтрокиУПП, ДанныеСтроки);
	КонецЕсли; 
	//>>
...
Процедура НачалоВыбора(Форма, Элемент, Модифицированность = Истина, СтандартнаяОбработка = Ложь) Экспорт
	...
	СтруктураКИУПП = MDMСервер.ПолучитьСтруктуруКИУПП();
	
	Если Строки.Количество() = 0 Тогда
		СтруктураКИУПП.Объект = Форма.Параметры.Ключ;
		СтруктураКИУПП.Тип = ДанныеСтроки.Тип;
		СтруктураКИУПП.Вид = ДанныеСтроки.Вид;
	Иначе	
		ЗаполнитьЗначенияСвойств(СтруктураКИУПП, Строки[0]);
	КонецЕсли; 
	ПараметрыОткрытия.Вставить("КонтактнаяИнформацияУПП", СтруктураКИУПП);
	//>>
...
Процедура ПредставлениеНачалоВыбораЗавершение(Знач РезультатЗакрытия, Знач ДополнительныеПараметры) Экспорт
	...
	Иначе	
		ДанныеСтрокиУПП = Строки[0];
	КонецЕсли; 
	ЗаполнитьЗначенияСвойств(ДанныеСтрокиУПП, РезультатЗакрытия.КонтактнаяИнформацияУПП);
	//>>
