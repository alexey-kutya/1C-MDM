...
//	КутьяАА ITRR
//	ЭтоТипXDTO = ТипЗнч(XDTOАдресРФ) = Тип("ОбъектXDTO");
	
	Если XDTOАдресРФ = Неопределено Или XDTOАдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ") Тогда
//	Если НЕ ЗначениеЗаполнено(XDTOАдресРФ) Или ЭтоТипXDTO И (XDTOАдресРФ.Тип() <> ФабрикаXDTO.Тип(ПространствоИмен, "АдресРФ")) Тогда
...
Функция ПредставлениеАдреса(Знач XDTOАдрес, Знач ВидИнформации) Экспорт
	...
	Если ВидИнформации.ВключатьСтрануВПредставление
//	КутьяАА ITRR		
		И НЕ ЭтоРоссийскийАдрес(АдресРФ) Тогда
//	КутьяАА ITRR		
