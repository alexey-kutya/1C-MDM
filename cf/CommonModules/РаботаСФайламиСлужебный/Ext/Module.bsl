...
Функция ТекстЗапросаДляСинхронизацииФайлов(ВладелецФайла, НастройкаСинхронизации, МассивИсключений, ЭлементИсключение) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник." + СправочникПапок.Имя + " КАК СправочникПапок
	...
	Если СправочникПапок.Иерархический Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	ВЫБОР
	...
	|	ИСТИНА,
	|	ЛОЖЬ,
	|	ЕСТЬNULL(СправочникПапокИзменения.Ссылка, НЕОПРЕДЕЛЕНО),
	...
	|	ИСТИНА
	|ИЗ
	|	Справочник." + СправочникПапок.Имя + " КАК СправочникПапок
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник." + СправочникПапок.Имя + ".Изменения КАК СправочникПапокИзменения
	|		ПО (СправочникПапокИзменения.Ссылка = СправочникПапок.Ссылка)
	|			И (СправочникПапокИзменения.Узел = &УчетнаяЗапись)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыСинхронизацииФайловСОблачнымСервисом КАК СтатусыСинхронизацииФайловСОблачнымСервисом
	...
	Если ЭлементИсключение <> Неопределено Тогда
		ТекстЗапроса = ТекстЗапроса + "
			|ГДЕ
			|	СправочникПапок.Ссылка В ИЕРАРХИИ (&ЭлементИсключение)";
	КонецЕсли;
