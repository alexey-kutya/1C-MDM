...
Функция РежимОбновленияДанныхВЛокальномРежимеРаботы()
	...
	Если РезультатВыполненияПакета[0].Пустой() И РезультатВыполненияПакета[1].Пустой() Тогда
		//Обновление с версии НСИ 1.0.4.1. Начало
		Если Константы.нсиЭтоНеПервыйЗапуск.Получить() Тогда   
			Возврат "ОбновлениеВерсии";
		Иначе
			//Обновление с версии НСИ 1.0.4.1. Конец
	...
	Для Каждого Подсистема Из ОписанияПодсистем.Порядок Цикл
		...
		Для Каждого ДеревоОбработчиковВерсия Из ДеревоОбработчиковБиблиотека.Строки Цикл
			...
			Для Каждого Обработчик Из ДеревоОбработчиковВерсия.Строки Цикл
				...
				Если Не ЭтоПодчиненныйУзелРИБ И Обработчик.ЗапускатьТолькоВГлавномУзле И РежимВыполнения = "Параллельно" Тогда
		...
		// В параллельном режиме в РИБе с фильтрами в подчиненном узле выполняются только обработчики
