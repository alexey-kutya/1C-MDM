...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураАдреса = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПолучитьАдресУПП(КонтактнаяИнформацияУПП);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СтруктураАдреса,,"ТипДома, ТипСтроения, ТипПомещения");
...
Процедура ТипДомаПриИзменении(Элемент)
	...
	//КутьяАА
	СформироватьПредставлениеАдресаУПП();
...
Процедура ПредставлениеИностранногоАдресаПриИзменении(Элемент)
	...
	//КутьяАА ITRR
	//ПредставлениеАдреса = Строка(Страна)+", "+ПредставлениеАдреса;
	//АдресУПП_Представление = ПредставлениеАдреса;
	//АдресУПП_Индекс_Поле1 = Строка(Страна);
...
Процедура ОбработкаИзмененияСтраныКлиент()
	...
	//КутьяАА ITRR
	Элементы.ГруппаАдресУПП.Видимость = ЭтоРоссийскийАдрес;
...
//	КутьяАА ITRR	
//становитьСписокВыбораЭлемента(Элементы.ТипДомаУПП, Элементы.АдресУПП_Дом_Поле7, ВариантыДанныхДом);
	УстановитьСписокВыбораЭлемента(Элементы.ТипДома1, Элементы.Дом1, ВариантыДанныхДом);
...
//	КутьяАА ITRR	
//	УстановитьСписокВыбораЭлемента(Элементы.ТипСтроенияУПП,  Элементы.АдресУПП_Корпус_Поле8, ВариантыДанныхСтроение);
	УстановитьСписокВыбораЭлемента(Элементы.ТипСтроения1,  Элементы.Строение1, ВариантыДанныхСтроение);
...
//	КутьяАА ITRR	
//	УстановитьСписокВыбораЭлемента(Элементы.ТипПомещенияУПП, Элементы.АдресУПП_Квартира_Поле9, ВариантыДанныхПомещение);
	УстановитьСписокВыбораЭлемента(Элементы.ТипПомещения1, Элементы.Помещение1, ВариантыДанныхПомещение);
	//
...
//
&НаКлиенте
Процедура ОбновитьАдресУПП(ИмяЭлемента = "")
	
	Если ИмяЭлемента = "" Тогда
		//	АдресУПП_Индекс_Поле1 = Индекс;
		АдресУПП_Регион_Поле2 = НаселенныйПунктДетально.Регион.Представление;
		АдресУПП_Округ = НаселенныйПунктДетально.Округ.Представление;
		АдресУПП_Район_Поле3 = НаселенныйПунктДетально.Район.Представление;
		АдресУПП_Город_Поле4 = НаселенныйПунктДетально.Город.Представление;
		АдресУПП_ВнутригородскойРайон = НаселенныйПунктДетально.ВнутригРайон.Представление;
		АдресУПП_НаселенныйПункт_Поле5 = НаселенныйПунктДетально.НаселенныйПункт.Представление;
		АдресУПП_Улица_Поле6 = Улица;
		//АдресУПП_Дом_Поле7 = Дом;
		//АдресУПП_Корпус_Поле8 = Строение;
		//АдресУПП_Квартира_Поле9 = Помещение;
	ИначеЕсли ИмяЭлемента = "НаселенныйПункт" Тогда
		АдресУПП_Регион_Поле2 = НаселенныйПунктДетально.Регион.Представление;
		АдресУПП_Округ = НаселенныйПунктДетально.Округ.Представление;
		АдресУПП_Район_Поле3 = НаселенныйПунктДетально.Район.Представление;
		АдресУПП_Город_Поле4 = НаселенныйПунктДетально.Город.Представление;
		АдресУПП_ВнутригородскойРайон = НаселенныйПунктДетально.ВнутригРайон.Представление;
		АдресУПП_НаселенныйПункт_Поле5 = НаселенныйПунктДетально.НаселенныйПункт.Представление;
	ИначеЕсли ИмяЭлемента = "Улица" Тогда
		АдресУПП_Улица_Поле6 = Улица;
	КонецЕсли; 
...
КонецПроцедуры // ОбновитьАдресУПП()

// Процедура формирует строковое представление адреса.
&НаКлиенте
Процедура СформироватьПредставлениеАдресаУПП(МенятьПредставлениеАдресаMDM = Ложь)
	
	ЗаписатьАдресУПП();
	АдресУППСтруктура = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПолучитьАдресУПП(КонтактнаяИнформацияУПП);
//	АдресУПП_Представление = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПолучитьПредставлениеАдресаУПП(АдресУППСтруктура);
	Если Страна = СтранаРоссия Тогда
		ПредставлениеАдреса = УправлениеКонтактнойИнформациейСлужебныйВызовСервера.ПолучитьПредставлениеАдресаУПП(АдресУППСтруктура);
	КонецЕсли; 
	
	//Если МенятьПредставлениеАдресаMDM Тогда
	//	ПредставлениеАдреса = АдресУПП_Представление;
	//КонецЕсли; 
...
&НаКлиенте
Процедура АдресУПП_Регион_Поле2ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Район_Поле3ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Город_Поле4ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_НаселенныйПункт_Поле5ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Улица_Поле6ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Дом_Поле7ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Корпус_Поле8ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Квартира_Поле9ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура АдресУПП_Индекс_Поле1ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура ОкругПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры
...
&НаКлиенте
Процедура АдресУПП_ДополнительнаяИнформация_Поле10ПриИзменении(Элемент)
	Модифицированность = Истина;
	СформироватьПредставлениеАдресаУПП(Истина);
КонецПроцедуры

&НаКлиенте
Процедура ЗаписатьАдресУПП()

	ЭтоРоссийскийАдрес = (Страна = СтранаРоссия);
	
//	КонтактнаяИнформацияУПП.Вставить("Представление", АдресУПП_Представление);
	КонтактнаяИнформацияУПП.Вставить("Представление", ПредставлениеАдреса);
	Если ЭтоРоссийскийАдрес Тогда
//		КонтактнаяИнформацияУПП.Вставить("Поле1", АдресУПП_Индекс_Поле1);
		КонтактнаяИнформацияУПП.Вставить("Поле1", Индекс);
		КонтактнаяИнформацияУПП.Вставить("Поле2", АдресУПП_Регион_Поле2);
		КонтактнаяИнформацияУПП.Вставить("Поле3", АдресУПП_Район_Поле3);
		КонтактнаяИнформацияУПП.Вставить("Поле4", АдресУПП_Город_Поле4);
		КонтактнаяИнформацияУПП.Вставить("Поле5", АдресУПП_НаселенныйПункт_Поле5);
		КонтактнаяИнформацияУПП.Вставить("Поле6", АдресУПП_Улица_Поле6);
		//КонтактнаяИнформацияУПП.Вставить("Поле7", АдресУПП_Дом_Поле7);
		//КонтактнаяИнформацияУПП.Вставить("Поле8", АдресУПП_Корпус_Поле8);
		//КонтактнаяИнформацияУПП.Вставить("Поле9", АдресУПП_Квартира_Поле9);
		КонтактнаяИнформацияУПП.Вставить("Поле7", Дом);
		КонтактнаяИнформацияУПП.Вставить("Поле8", Строение);
		КонтактнаяИнформацияУПП.Вставить("Поле9", Помещение);
		КонтактнаяИнформацияУПП.Вставить("Поле10", АдресУПП_ДополнительнаяИнформация_Поле10);
		КонтактнаяИнформацияУПП.Вставить("Округ", АдресУПП_Округ);
		КонтактнаяИнформацияУПП.Вставить("ВнутригородскойРайон", АдресУПП_ВнутригородскойРайон);
		КонтактнаяИнформацияУПП.Вставить("ТипДома", ТипДома);
		КонтактнаяИнформацияУПП.Вставить("ТипСтроения", ТипСтроения);
		КонтактнаяИнформацияУПП.Вставить("ТипПомещения", ТипПомещения);
	Иначе
//		КонтактнаяИнформацияУПП.Вставить("Поле1", АдресУПП_Индекс_Поле1);
		КонтактнаяИнформацияУПП.Вставить("Поле1", Строка(Страна));
		КонтактнаяИнформацияУПП.Вставить("Поле2", "");
		КонтактнаяИнформацияУПП.Вставить("Поле3", "");
		КонтактнаяИнформацияУПП.Вставить("Поле4", "");
		КонтактнаяИнформацияУПП.Вставить("Поле5", "");
		КонтактнаяИнформацияУПП.Вставить("Поле6", "");
		КонтактнаяИнформацияУПП.Вставить("Поле7", "");
		КонтактнаяИнформацияУПП.Вставить("Поле8", "");
		КонтактнаяИнформацияУПП.Вставить("Поле9", "");
		КонтактнаяИнформацияУПП.Вставить("Поле10", "");
		КонтактнаяИнформацияУПП.Вставить("Округ", "");
		КонтактнаяИнформацияУПП.Вставить("ВнутригородскойРайон", "");
		КонтактнаяИнформацияУПП.Вставить("ТипДома", "");
		КонтактнаяИнформацияУПП.Вставить("ТипСтроения", "");
		КонтактнаяИнформацияУПП.Вставить("ТипПомещения", "");
	КонецЕсли; 
	
	Если ВидКонтактнойИнформации.Ссылка = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресКонтрагента")
		ИЛИ ВидКонтактнойИнформации.Ссылка = ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресОрганизации") Тогда
		КонтактнаяИнформацияУПП.Вставить("ЗначениеПоУмолчанию", Истина);
	КонецЕсли; 
