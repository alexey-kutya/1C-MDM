...
Процедура ЗагрузитьПравила(Команда)
	...
	//ПараметрыДиалога = Новый Структура; //фыв--
	...
	Фильтр="Файл правил обмена(*.xml)|*.xml";
...
Процедура ЗагрузитьНаСервере(Адрес)
	...
	Путь = КаталогВременныхФайлов() + "ExchangeXML.xml"; 
	ДвоичныеДанные.Записать(Путь);	
	
	Запись=РегистрыСведений.ПравилаОбмена.СоздатьМенеджерЗаписи();
	Запись.ПравилаОбмена="ExchangeXML";
	Запись.ПравилоВыгрузки=Новый ХранилищеЗначения(Путь);
	Попытка
		Запись.Записать();
	Исключение
		Сообщить("Не удалось записать правила обмена!");
	КонецПопытки;
